server <- function(input, output, session) {
  ...
  output$map_plot2 <- renderPlotly({
    ...
    plot_ly(x = ~temp$dyn_calc, y = ~reorder(temp$name_location, temp$dyn_calc), type = 'bar', orientation = 'h',
      ...
      layout(title = FALSE, xaxis = list(title = "Значение"), yaxis = list(title = ""))%>%
      layout(
        images = list(
  ...
  output$map_plot3 <- renderPlotly({
    ...
    if(!is.null(ind) & !is.null(loc)){
      ...
      plot_ly() %>%
        ...
        layout(title = FALSE, xaxis = list(title = "Значение"), yaxis = list(title = "Стабильность, %")) %>%
        layout(
          images = list(
            source = base64enc::dataURI(file = '3.img/15.png'),
            x = 0.82, y = 1.05 ,
  ...
  output$Draggable_Graph <- renderPlotly(
    {
      ...
      plot_ly() %>%
        add_lines(x = k$year, y = k$dyn_calc,mode = 'lines', name = 'Инерционный сценарий', color = I("deepskyblue")) %>%
        add_lines(x = k2$year, y = k2$dyn_calc,mode = 'lines', name = 'Ваш сценарий', color = I("orange")) %>%
        ...
        add_annotations(
          text = paste0(input$f,", ", spravka$unique_name[which(spravka$id == indicator$unit_id[which(indicator$long_name == input$f)])]),
          x = 0,
          y = 1,
          yref = "paper",
          xref = "paper",
